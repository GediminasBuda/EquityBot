...
- `gemini` + a Gemini model id (e.g. `gemini-2.5-pro`, `gemini-2.5-flash`) — requires `GEMINI_API_KEY`. Routes through Google's OpenAI-compatible endpoint at `https://generativelanguage.googleapis.com/v1beta/openai/` (hardcoded in `llm_client.py`, `_openai()` with a custom `base_url`, same pattern as DeepSeek/Kimi). Added 2026-07-23, pre-emptively carrying the same two safety nets built for Kimi, since Gemini 2.5 Pro is also a "thinking" model with hidden reasoning tokens before the final answer: `GEMINI_MAX_TOKENS_MULTIPLIER` (default `2.0`) / `GEMINI_MAX_TOKENS_CAP` (default `16000`) scale every call's `max_tokens` inside the `gemini` branch of `generate()`; `GEMINI_REQUEST_TIMEOUT_SECONDS` (default `240`) bounds each call so a slow/hung completion raises a catchable timeout instead of risking a silent Streamlit Cloud worker kill. Not yet live-tested — if the actual failure modes differ from Kimi's, tune these constants (or add new ones) rather than changing the `claude`/`openai`/`deepseek`/`kimi` branches.
