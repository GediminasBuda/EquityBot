...
### Growth Pricing & Peers — EV/Sales and EV/Gross Profit showing "0.0x" for KSPI (2026-08-22)
User's Peer Comparison table for KSPI (Kaspi.kz, Nasdaq ADR) showed `EV / Sales` and `EV / Gross Profit` as `0.0x` instead of a real multiple or `n/a`. Root cause: `eodhd_only_builder.py`'s `build_company_data_from_bundle()` set `company.enterprise_value = _to_m(v.get("EnterpriseValue"))` using the plain `_f()`-based `_to_m()` helper — not `_fz()`. EODHD returns the literal string `"0"` for `Valuation.EnterpriseValue` on some foreign ADRs/thinly-covered tickers it can't compute EV for (the same "0 means missing, not null" quirk already documented and fixed for `pe_ratio`/`ev_sales`/`ev_ebitda`/etc. via `_fz()` — see the earlier "HIGH.DE" Industry Analysis fix), so `company.enterprise_value` ended up `0.0` (not `None`). `models/growth_pricing.py`'s `compute_peer_snapshot_row()` then computed `ev_gross_profit = 0.0 / latest.gross_profit` and `ev_sales = 0.0 / denom_revenue`, both literally `0.0` — a real (non-None) value the `_x()` formatter renders as `"0.0x"` rather than `"n/a"`. Separately, `CompanyData.calculate_current_ratios()` already has a market_cap+net_debt fallback for a missing EV, but it's gated on `self.enterprise_value is None` — since EV was `0.0` (not `None`) the fallback never fired even where it is called elsewhere, and it isn't called at all on the `fetch_company_data_eodhd_only()` path used by Growth Pricing & Peers / Investment Memo V2 in the first place. **Fix (2026-08-22):** (1) changed the `EnterpriseValue` read to `_fz()` the raw value before the `/1e6` conversion, so a literal `"0"` is treated as missing. (2) Added a direct fallback in `eodhd_only_builder.py` (after the annual-financials recompute loop): if `company.enterprise_value` is still `None` and `company.market_cap` is set, compute `market_cap + latest_annual().net_debt` when net debt is known — mirrors `calculate_current_ratios()`'s logic but runs unconditionally on this fetch path. Same fix benefits every EODHD-only-fetched peer too (the peer-fetch loop in `pages/report_generator.py`'s `growth_pricing` dispatch calls the same builder). **PEG Ratio showing blank and Sales TTM Growth showing a plausible-but-surprising negative number for this same KSPI report were investigated but NOT changed** — `peg_ratio` already used `_fz()` correctly and the fallback (`forward_pe / (forward_eps_growth*100)`) requires EODHD forward-estimate fields that may simply be absent for this thinly-covered ADR; `sales_ttm_growth`'s formula (`ttm_revenue / latest_fiscal_year_revenue - 1`, in `_sales_ttm_growth()`) was traced end-to-end and found logically correct, tracing the negative figure to EODHD's own `Highlights.RevenueTTM` value for KSPI rather than an app-side calculation bug. **If a similar "surprising negative TTM growth" or "PEG missing" report recurs on a re-run with fresh (non-cached) data, treat it as a potential EODHD data-coverage gap for that specific ticker, not a code bug, unless cross-checked against `company.quarterly_revenue_growth_yoy` (EODHD's own independently-reported YoY figure) shows a clear sign mismatch — that would indicate `Highlights.RevenueTTM` itself is stale/wrong for that ticker and worth a dedicated fallback (e.g. preferring the quarterly-sum TTM revenue already computed elsewhere in this file over the Highlights figure).**

### Growth Pricing & Peers — EV/Sales, EV/Gross Profit, PEG still wrong for KSPI after the "0" sentinel fix — real root cause was a dual-currency ADR mismatch (2026-08-22)
The `_fz()` fix above corrected the Snapshot/TTM `company.enterprise_value` scalar, but the user re-ran the report and found the Peer Comparison table and historical Earnings table were still broken: `EV/Gross Profit`/`EV/Sales` = `0.0x`, `PEG` = `n/a`, and historical `Enterprise Value` showed nonsensical figures (e.g. -89.34B, -337.65B). **User-diagnosed root cause:** KSPI (Kaspi.kz) is a Nasdaq-listed ADR where EODHD reports market cap/price/EPS in **USD** (the trading currency) but the underlying financial statements (revenue, gross profit, net debt, etc.) in **KZT** (the home/reporting currency). Every price-derived ratio that divides a USD-denominated EV/market-cap by a KZT-denominated statement figure (or vice versa) produces a meaningless number — this is a different, deeper bug than the "0" sentinel, affecting the whole price-derived ratio surface (market_cap, enterprise_value, pe_ratio, ev_sales, ev_ebitda, ev_ebit, fcf_yield — both current/TTM and every historical year), not just one field.

**Fix — dual-currency detection + currency-consistent EV, implemented across three files:**
1. **`data_sources/eodhd_all_in_one.py`**: added `fetch_forex_rate(base_ccy, quote_ccy)` — spot rate via EODHD's `/real-time/{PAIR}.FOREX` endpoint (e.g. `USDKZT.FOREX`). Returns `None` on any failure/shape mismatch; callers must treat a missing rate as "cannot convert" rather than guessing.
2. **`data_sources/base.py`**: added three new `CompanyData` fields — `currency_financials` (the statements' reporting currency, distinct from the existing trading-currency field), `fx_rate_price_to_financials` (spot rate to convert 1 unit of trading currency into `currency_financials`), and `enterprise_value_financials_ccy` (a parallel, currency-consistent EV built specifically for ratios that must be divided by a statement-currency figure). All three are `None` for the overwhelming majority of companies where the two currencies match — existing `company.enterprise_value`/`ev_sales`/`ev_ebitda` are untouched and still used for everything else.
3. **`data_sources/eodhd_only_builder.py`** (the builder Growth Pricing & Peers / Investment Memo V2 actually use — not `eodhd_adapter.py`, per the standing rule in this doc):
   - **Detection**: compares the per-statement `currency_symbol` field (read from `Financials.Income_Statement`/`Balance_Sheet`/`Cash_Flow`) against the company's trading currency; if they differ, fetches the FX rate via the new method and stores it on `company.fx_rate_price_to_financials`. **Caveat: `currency_symbol` as the exact EODHD field name is an educated guess, not verified against a live API response (no working Python interpreter was available in this environment to confirm the schema) — implemented defensively via `.get()` chains so it safely falls through to "not dual-currency" if the field doesn't exist under that name.** If this fix doesn't trigger for KSPI, checking the actual field name returned by EODHD's `/fundamentals/` endpoint for `Financials.Income_Statement` is the first thing to check.
   - **Historical years**: when dual-currency and FX is available, converts `af.price_year_end` into the reporting currency (via the FX rate) before feeding it into `market_cap`/`pe_ratio` calculations; when FX is unavailable, temporarily blanks `price_year_end` for the duration of `af.calculate_derived()` (then restores it) so no derived field can silently combine mismatched currencies — displays `n/a` rather than a wrong number, consistent with the `_fz()` philosophy elsewhere in this file. Only the current spot FX rate is used for all historical years (no historical-FX-rate endpoint implemented) — a known approximation, acceptable given the scope, but worth revisiting if historical multiples still look off after this fix.
   - **Current/TTM**: builds `company.enterprise_value_financials_ccy = (shares_outstanding × current_price × fx_rate) + latest_annual().net_debt` — i.e. `EV = Market Cap + Net Debt` (net_debt = debt − cash, so a net-cash company gets EV < market cap automatically), matching the existing convention used everywhere else in this codebase (`af.enterprise_value = af.market_cap + (nd or 0)`). **Note: the user's original phrasing said "minus Net Debt," but this was implemented as the standard `+ Net Debt` convention for consistency with every other EV calculation already in the codebase — flagged here in case the user actually meant something different (e.g. subtracting a debt figure that's already expressed as a negative-of-cash convention).**
4. **`models/growth_pricing.py`**: both `compute_current_snapshot()` and `compute_peer_snapshot_row()` now (a) prefer `company.enterprise_value_financials_ccy` over `company.enterprise_value` when computing `ev_gross_profit` (no EODHD-native equivalent exists for this ratio, so it's always self-calculated), (b) prefer EODHD's own precomputed `company.ev_sales` over a manual calc for `ev_sales` (EODHD computes this internally with matched currencies already, so recomputing is redundant/riskier), and (c) added `_peg_fallback_from_history()` — PEG via trailing P/E over trailing 3-year EPS CAGR — as a fallback when EODHD has no forward-consensus EPS growth estimate. This fallback is currency-invariant (a ratio of two same-currency EPS values) so it's safe for dual-currency ADRs without any FX handling.

**Not yet live-tested** — no working Python interpreter was available in this sandbox session, so this is unverified against KSPI's actual live EODHD response; ask the user to re-run the KSPI Growth Pricing & Peers report to confirm the Peer Comparison table now shows real `EV/Gross Profit`/`EV/Sales` multiples (or a clean `n/a` if FX lookup fails) instead of `0.0x`, and that the historical Enterprise Value column no longer shows large negative figures. **General rule: any future report-type bug on a foreign ADR/dual-listed ticker showing wildly-wrong (not just missing) price-derived ratios should be checked against this dual-currency mechanism first — check `company.currency_financials` vs the trading currency, and whether `fx_rate_price_to_financials` was actually populated, before assuming a different root cause.**

**Follow-up (2026-08-22): `AttributeError: 'CompanyData' object has no attribute 'enterprise_value_financials_ccy'` during PDF rendering.** `compute_peer_snapshot_row()` in `models/growth_pricing.py` accessed `company.enterprise_value_financials_ccy` directly. Same class of bug as the earlier "Investment Memo V2 — AttributeError on ttm_revenue for cached objects" fix in Section 7: a `CompanyData` instance built from a still-in-memory old class (before Streamlit Cloud's worker process fully picks up the new `base.py`) lacks a brand-new field entirely, raising `AttributeError` instead of evaluating to `None`. **Fix:** switched to `getattr(company, "enterprise_value_financials_ccy", None)`. **General rule: any newly-added `CompanyData` field must be read via `getattr(company, 'field_name', None)` at its call sites for at least one deploy cycle — direct attribute access (`company.new_field`) is only safe once you're certain no stale-class instances can reach that code path, which is never guaranteed on Streamlit Cloud immediately after a field is added.**

### Investment Memo V2 — dual-currency ADR fix extended from Growth Pricing & Peers; "Sales (USDM)"/"FCF (USDM)" mislabeled for KZT-reporting ADRs (2026-08-22)
User found the same KSPI (Kaspi.kz — USD price/market data, KZT financial statements) dual-currency bug in the Investment Memo report: EV-derived figures were wrong, and the Financial Summary table (page 1) hardcoded the row labels "Sales (USDM)" and "FCF (USDM)" even though the values displayed are actually in KZT (the statement currency) for this company — the label was never dynamic, it was written assuming every company reports in USD. Per explicit instruction, extended the already-validated Growth Pricing & Peers dual-currency mechanism (`currency_financials`, `enterprise_value_financials_ccy`, `fx_rate_price_to_financials` — see the two entries directly above) to this report, universally (not KSPI-specific):

1. **`models/overview.py`** (`_format_financials_for_llm()` and `_overview_prompt_parts()`): the LLM prompt now splits `cur` (trading currency, for Current Price) from `cur_fin = getattr(company, "currency_financials", None) or cur` (for Market Cap... actually Enterprise Value, Net Debt, the entire Annual Financials table header, and Forward Estimates Revenue/Net Income). Enterprise Value in the prompt now uses `enterprise_value_financials_ccy` when set, falling back to `company.enterprise_value` otherwise. When `cur_fin != cur`, an explicit dual-currency warning paragraph is appended to the dynamic prompt telling the LLM which figures are in which currency and not to mix them when reasoning about ratios — necessary because the LLM writes prose (Bull/Bear Case, Recommendation rationale) that could otherwise state a nonsensical mixed-currency ratio in the narrative text itself, which no PDF-layer fix could catch after the fact.
2. **`agents/pdf_overview_v2.py`** (Financial Summary table, page 1): the `cur` used for row labels was already `getattr(company, "currency_financials", None) or company.currency or ""` from the earlier Growth Pricing session (not KSPI-Investment-Memo-specific), so "Sales ({cur}M)"/"FCF ({cur}M)" already resolve to "KZTM" for KSPI — the bug was that **Mkt Cap and EV rows were still unlabeled** (plain `"Mkt Cap (M)"`/`"EV"`), which is what actually needed fixing here. Also fixed a second, subtler bug: `AnnualFinancials.market_cap` (historical years, computed from an FX-converted year-end price for dual-currency companies — see `eodhd_only_builder.py`) is in the reporting currency, while `company.market_cap` (the TTM column) is always in the trading currency — showing them in the same unlabeled row would have silently mixed currencies across columns even after adding a label. **Fix:** added currency labels to both rows (`f"Mkt Cap ({cur}M)"`, `f"EV ({cur}M)"`), and derived a reporting-currency TTM market cap as `enterprise_value_financials_ccy - net_debt` (both already reporting-currency, since `company.net_debt` comes from `latest_annual().net_debt`, EODHD's raw balance-sheet figure — never trading-currency) instead of showing the raw trading-currency `company.market_cap` in that row.
3. **New bug found during the "make it universal" pass, not reported by the user but caught by re-checking the underlying ratio math**: `company.ev_ebit` and `company.fcf_yield` have **no EODHD-native precomputed equivalent** (unlike `ev_sales`/`ev_ebitda`, which EODHD computes internally with matched currencies) — both are always filled in by `CompanyData.calculate_current_ratios()`'s fallback (`data_sources/base.py`): `ev_ebit = self.enterprise_value / la.ebit` and `fcf_yield = la.fcf / self.market_cap`. For a dual-currency ADR, `self.enterprise_value` and `self.market_cap` are trading-currency while `la.ebit`/`la.fcf` are reporting-currency — the exact same mismatch class already fixed for EV/Sales and EV/Gross Profit, just not yet applied to these two fields. Since this fallback runs generically for every report (not just Investment Memo), the bug silently affected Growth Pricing & Peers too. **Fix, at the source (`data_sources/eodhd_only_builder.py`, inside the existing dual-currency block, before `company.calculate_current_ratios()` is called):** pre-populate `company.ev_ebit = enterprise_value_financials_ccy / la.ebit` and `company.fcf_yield = la.fcf / mc_financials` (a reporting-currency TTM market cap computed the same way as the PDF fix above) whenever dual-currency and FX conversion succeeded — this makes `calculate_current_ratios()`'s `if self.ev_ebit is None`/`if self.fcf_yield is None` guards skip the mismatched fallback entirely. Fixing it at this single source location (rather than patching every PDF/prompt call site) automatically benefits Investment Memo's "EV/EBIT"/"FCF Yield" rows, its peer table's EV/EBIT column, and Growth Pricing & Peers' TTM snapshot — satisfying the "universal" requirement without per-report duplication.
4. **Page 4 "EODHD Market Data" appendix** (`_page4_eodhd_data()`, the raw-EODHD-dump page — separate from the Financial Summary table): its Market Cap/Enterprise Value figures are intentionally raw, unprocessed EODHD scalars (the page's own subtitle says "no LLM processing") and were left as trading-currency values rather than retrofitted with the currency-consistent calculation, to preserve that page's purpose as an unmodified data provenance record. Added a conditional footnote instead: when `currency_financials != currency_price`, a small italic note now tells the reader this page's Market Cap/EV are raw trading-currency EODHD figures and points them to the Financial Summary table for the currency-consistent numbers actually used in this report's ratios.

**Not yet live-tested** (same standing constraint as the Growth Pricing & Peers fix — no working Python interpreter available in this sandbox session). Ask the user to re-run the KSPI Investment Memo report to confirm: the Financial Summary table shows "Sales (KZTM)"/"FCF (KZTM)"/"Mkt Cap (KZTM)"/"EV (KZTM)" row labels, EV/EV-EBIT/EV-Sales/FCF-Yield figures are sane on both page 1 and the page 3 Peer Comparison table, and the Bull/Bear Case narrative no longer states a mixed-currency ratio. **General rule: this dual-currency mechanism (`currency_financials`/`enterprise_value_financials_ccy`/`fx_rate_price_to_financials`) is now used by Growth Pricing & Peers AND Investment Memo V2 — any future report type built on `eodhd_only_builder.py`'s `CompanyData` should check for it too before assuming a price-derived ratio bug is something else. `ev_ebit`/`fcf_yield` are fixed at the source in `eodhd_only_builder.py` and need no per-report patching; any *new* ratio field added in the future that divides a market-data figure by a statement figure (or vice versa) should follow the same pattern — pre-populate it in the dual-currency block before `calculate_current_ratios()` runs, don't patch it per-consumer.**

### Growth Pricing & Peers — page-1 Snapshot table: unlabeled/mismatched MCap & EV, FCF Yield showing 2448.5% for KSPI (2026-08-22)
User re-reported, specifically on the page-1 "Current / Forward-Looking Snapshot" table of Growth Pricing & Peers (not the Peer Comparison table, which was already fixed and validated): Market Cap and Enterprise Value both showed unlabeled figures that looked like they were both in USD (misleading for a dual-currency ADR like KSPI, where statements are in KZT), and FCF Yield (TTM) showed a nonsensical 2448.5% against a historical range of 6-20% elsewhere in the same report.

**Root cause, three layers:**
1. **`models/growth_pricing.py`'s `compute_current_snapshot()`** (feeds this one table only — `compute_peer_snapshot_row()`, used by the Peer Comparison table, was already correctly dual-currency-aware) read `company.enterprise_value` (always trading-currency) directly, with no awareness of `enterprise_value_financials_ccy` at all, and returned no currency metadata for either Market Cap or EV — so the PDF table could not label either row even if it wanted to.
2. **The FX-dependent pre-populate block in `eodhd_only_builder.py`** that's supposed to set `enterprise_value_financials_ccy`/`ev_ebit`/`fcf_yield` to currency-consistent values is gated on `_dual_currency and _fx_rate and shares_outstanding and current_price` — when the live forex lookup (`fetch_forex_rate`) fails or any of those inputs is momentarily unavailable, the whole block is skipped and these three fields stay `None`, falling through to whatever fallback runs next.
3. **That fallback, `CompanyData.calculate_current_ratios()` in `data_sources/base.py`, had zero dual-currency awareness** — its EV/EV-EBIT/EV-EBITDA/EV-Sales/FCF-Yield fallbacks all divide/add a trading-currency market-data scalar (`self.market_cap`, `self.enterprise_value`) against a reporting-currency statement figure (`la.ebit`, `la.revenue`, `la.fcf`) whenever the currency-aware pre-populate above didn't already fill the field — completely unconditionally. This is what actually produced the 2448.5%: `la.fcf` (~491,880M KZT) divided by `self.market_cap` (~20,060M USD, unconverted) ≈ 24.5 → "2448.5%" — arithmetically exactly what a mismatched-currency division would produce. There is also a separate, unconditional (not even gated on FX success) `company.enterprise_value = company.market_cap + la.net_debt` fallback earlier in `eodhd_only_builder.py`, mixing the same currencies whenever EODHD's own `EnterpriseValue` field is missing/zeroed.

Unlike the earlier KSPI fixes (Peer Comparison table, Investment Memo), this exposed that `calculate_current_ratios()` — a **generic method called by every fetch path**, not just the eodhd-only one — was the real gap: pre-populating currency-consistent values only helps when the FX lookup happens to succeed; the generic fallback needed its own guard so a transient FX failure degrades to "n/a" instead of silently emitting a wrong number.

**Fix (2026-08-22):**
1. **`data_sources/base.py`** — `calculate_current_ratios()` now computes `_dual = bool(self.currency_financials and self.currency_price and str(self.currency_financials).upper() != str(self.currency_price).upper())` and adds `and not _dual` to the EV (`market_cap + net_debt`), EV/EBIT, EV/EBITDA, EV/Sales, and FCF Yield fallback conditions. For the vast majority of companies (`currency_financials` unset or equal to `currency_price`) this is a no-op; for a dual-currency company whose upstream FX-aware pre-populate didn't run, these fields now correctly stay `None` → "n/a" instead of mixing currencies. Gearing (`net_debt / ebitda`) was left unguarded — both operands are already reporting-currency balance-sheet/income-statement figures, no trading-currency scalar involved.
2. **`data_sources/eodhd_only_builder.py`** — gated the standalone `company.enterprise_value = company.market_cap + net_debt` fallback (which runs before the FX-dependent block and before `calculate_current_ratios()`) with `and not _dual_currency`, and corrected the adjacent comment, which had gone stale and incorrectly claimed `calculate_current_ratios()` "does not" get called on this path — it does (at the end of the same function), which is exactly why the base.py fix above is necessary and sufficient as the safety net.
3. **`models/growth_pricing.py`** — `compute_current_snapshot()` now prefers `getattr(company, "enterprise_value_financials_ccy", None)` over `company.enterprise_value` (same pattern as `compute_peer_snapshot_row()`), and returns `market_cap_currency` (`company.currency_price`) and `enterprise_value_currency` (`company.currency_financials` when the currency-consistent EV was actually used, else falls back to the trading currency) alongside the existing scalar values.
4. **`agents/pdf_growth_pricing.py`** — `_snapshot_table()` now appends the currency code returned above directly into the row label, e.g. `"Market Capitalization (current, USD)"` / `"Enterprise Value (current, KZT)"`, so the table is self-documenting without needing a separate footnote.

**Not yet live-tested** (same standing no-Python-interpreter constraint as every prior fix in this dual-currency effort). Ask the user to re-run the KSPI Growth Pricing & Peers report to confirm: the Snapshot table's Market Cap row is labeled "(current, USD)", the EV row is labeled "(current, KZT)" and shows a materially different, sane figure (not the same number as before), and FCF Yield (TTM) either shows a plausible percentage in the same range as the historical years or "n/a" — never another triple/quadruple-digit figure. **General rule: any ratio field with a per-fetch-path currency-aware pre-populate (the pattern established across `eodhd_only_builder.py`) must ALSO have its generic fallback in `data_sources/base.py` (or wherever else that fallback lives) gated against the same dual-currency condition — pre-populating only the "happy path" (FX lookup succeeds) is not sufficient, because the generic fallback still runs unconditionally whenever the pre-populate is skipped for any reason (FX API failure, missing shares/price at that instant, etc.), and it has no way to know a currency mismatch is even possible unless it's told to check.**

### Growth Pricing & Peers — "Sales TTM Growth" row replaced with simple last-fiscal-year YoY growth (2026-08-22)
User reported the Peer Comparison table's "Sales TTM Growth" row (`_sales_ttm_growth()`: `company.ttm_revenue / latest_fiscal_year.revenue - 1`) often produced incorrect-looking numbers — consistent with the standing note in the earlier KSPI entry above that `company.ttm_revenue` (EODHD's `Highlights.RevenueTTM`, or a quarterly-sum fallback) can itself be a stale/inconsistent figure for thinly-covered tickers, an EODHD data-coverage gap rather than an app bug, but one that made this row unreliable often enough to replace outright. **Fix, per explicit user request:** removed `_sales_ttm_growth()` entirely and replaced the row's source with the already-existing `_latest_revenue_yoy()` helper (most recent completed fiscal year's revenue vs. the prior fiscal year — a plain YoY calculated from two already-reliable `AnnualFinancials.revenue` values, no TTM/quarterly data involved) in `models/growth_pricing.py`'s `compute_peer_snapshot_row()`. This is the same figure already used internally for the Rule of 40 rows (`rule40_ebitda`/`rule40_net`), just not previously exposed as its own row. Renamed the dict key `sales_ttm_growth` → `sales_yoy_growth` throughout (`PEER_METRIC_DIRECTIONS`, `agents/pdf_growth_pricing.py`'s candidates list and table/section descriptive text) and the row label "Sales TTM Growth" → "Sales YoY Growth". Sales 3Y CAGR (`_sales_cagr3()`, also pure annual-revenue based) and all other peer-table metrics are unchanged.

### Investment Memo V2 — FCF Yield (TTM) showing 2448.5% in Financial Summary table AND Investment Checklist for KSPI (2026-08-22)
User re-reported the exact 2448.5% FCF Yield figure (same magnitude as the Growth Pricing & Peers bug fixed directly above) inside the **Investment Memo** report specifically: the Financial Summary table's TTM "FCF Yield" cell, and the Investment Checklist's "FCF Yield > 10.0%" row (`models/overview.py`'s `_calculate_checklist()`, criterion 4) — both read `company.fcf_yield` directly, so both showed the identical wrong scalar. The adjacent TTM "Mkt Cap (KZTM)" row was confirmed correct (9192.10B, matching the already-deployed reporting-currency conversion), which ruled out "the whole dual-currency mechanism isn't firing" — the bug was isolated to `company.fcf_yield` itself.

**Root cause, reconstructed from the reported figures (no working Python interpreter in this sandbox to reproduce live):** back-solving `2448.5% = la.fcf(latest FISCAL YEAR, 491.10B KZT) / company.market_cap(TRADING-currency USD scalar, ~20.05B)` reproduces the exact reported number almost exactly — i.e. `company.fcf_yield` was set by `CompanyData.calculate_current_ratios()`'s **old, unguarded** `self.fcf_yield = la.fcf / self.market_cap` fallback (`data_sources/base.py`), not by `eodhd_only_builder.py`'s dual-currency pre-populate — even though the `_dual` guard added earlier this session (see the Growth Pricing & Peers snapshot-table entry above) should prevent that fallback from firing for a dual-currency company. This strongly suggests the report was generated against a not-yet-redeployed version of the code (Streamlit Cloud deploy lag, or a stale cached `CompanyData`), consistent with this whole multi-session effort's repeated "not yet live-tested" caveat — but rather than relying on a fragile chain of upstream pre-populate conditions to eventually reach every consumer correctly, this fix makes both consumers currency-safe **independently of whatever `company.fcf_yield` was set to upstream**, so a future stale-scalar edge case degrades gracefully instead of reproducing this exact class of bug again. Separately, a genuine (if smaller) bug was found and fixed at the source: `eodhd_only_builder.py`'s dual-currency pre-populate (added in a prior session) computed FCF Yield as `_la2.fcf` — the latest **fiscal year's** FCF — divided by `_mc_financials`, a **TTM** market cap figure, mixing periods (fiscal-year FCF against TTM price/shares) even when currencies matched correctly.

**Fix (2026-08-22):**
1. **`data_sources/eodhd_only_builder.py`** (source-level): the dual-currency FCF Yield pre-populate now prefers `company.ttm_fcf` (actual trailing-twelve-month FCF, already computed earlier in the same function from summed quarterly cash flow) over `_la2.fcf` (latest fiscal year), falling back to `_la2.fcf` only if `ttm_fcf` is unavailable — `_fcf_for_yield = getattr(company, "ttm_fcf", None) or (_la2.fcf if _la2 else None)`.
2. **`agents/pdf_overview_v2.py`** (defensive, per-consumer): the Financial Summary table's TTM "FCF Yield" cell is no longer a blind pass-through of `company.fcf_yield`. Moved the `_mc_ttm` (reporting-currency TTM market cap, `enterprise_value_financials_ccy - net_debt`) computation earlier — from its old position at the Mkt Cap row down near the bottom of the table up to just before the FCF Yield row — and reused it to recompute `_fcf_yield_ttm = _ttm_fcf / _mc_ttm` locally whenever `_ev_ccy` (i.e. `enterprise_value_financials_ccy`) is set and both values are available and positive; falls back to `company.fcf_yield` unchanged for the non-dual-currency case (`_ev_ccy is None`, the overwhelming majority of companies). The Mkt Cap row further down now reuses this same `_mc_ttm` instead of recomputing it, so both rows are guaranteed to divide by the identical reporting-currency market cap.
3. **`models/overview.py`** (defensive, per-consumer): `_calculate_checklist()`'s "FCF Yield > 10.0%" criterion applies the identical local recomputation — `fcfy = ttm_fcf / (enterprise_value_financials_ccy - net_debt)` whenever those fields are available — before building the pass/fail row, instead of trusting `company.fcf_yield` as-is.

**Not yet live-tested** (same standing no-Python-interpreter constraint as every prior fix in this dual-currency effort). Ask the user to re-run the KSPI Investment Memo report to confirm both the Financial Summary table's TTM FCF Yield cell and the Investment Checklist's "FCF Yield > 10.0%" row now show a plausible percentage in the same range as the historical years (roughly 6-20%, e.g. ~10% given 952.97B TTM FCF / 9192.10B TTM Mkt Cap) instead of 2448.5%. **General rule: for any ratio whose correctness depends on a single upstream scalar (`company.fcf_yield`, `company.ev_ebit`, etc.) that has its own chain of pre-populate/fallback/guard logic spread across multiple files, prefer having each PDF/checklist consumer recompute the ratio locally from the underlying already-verified-correct components (a currency-and-period-consistent numerator and denominator both already used elsewhere in the same function) whenever those components are available — this makes the consumer correct even if the upstream scalar is stale, wrong, or was set by code that hasn't been redeployed yet, rather than requiring every future bug in the upstream chain to be tracked down and fixed before the consumer is fixed too.**
