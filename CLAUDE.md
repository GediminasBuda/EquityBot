...
Each portfolio card's "📰 Latest news" expander only pulled from EODHD `/news` (or yfinance `.news` for Japan/Baltic) — both are third-party aggregators whose most recent item can be days or weeks stale for thinly-covered tickers, which isn't useful for spotting a fresh regulatory disclosure or press release. **Fix:** added `LLMClient.find_ir_announcements(company_name, ticker, min_items=5)` in `agents/llm_client.py` — uses the provider's **native web-search tool** (`_claude_web_search()` for Claude, `_openai_web_search()` for OpenAI) to locate the company's own Investor Relations "Press Releases"/"Ad Hoc Announcements" page (explicitly instructed to target the company's own `investors.`/`ir.` subdomain, not aggregators) and return up to 5 recent items as `{"title", "link", "date"}`, parsed from the model's final JSON-array text response the same way `generate_json()` parses JSON (find `[`...`]`, `json.loads(strict=False)`). **Only providers with a real web-search tool are used (`claude`, `openai`) — every other provider (`deepseek`/`kimi`/`gemini`/`xai`) returns `[]` immediately**, since none of them can verify a URL actually exists before returning it, and a fabricated-looking IR link would be worse than showing nothing (same "degrade gracefully, never invent" principle as the rest of `llm_client.py`). Wired into `pages/my_portfolio.py` via `_fetch_ir_announcements()` (`st.cache_data(ttl=21600)` — 6h, since LLM web search is far more expensive per call than the plain news endpoints and an IR page doesn't turn over minute-to-minute) called with `name_full` (the card's already-resolved company name) and `ticker`. Rendered as a new "INVESTOR RELATIONS / AD HOC ANNOUNCEMENTS" block (blue `IR` badge) above the existing EODHD/yfinance list, which is now labeled "OTHER NEWS" beneath it when both are present. **General rule: this reuses `_claude_web_search()`/`_openai_web_search()` as-is (no new HTTP client code) — if either provider's underlying method's `max_tokens` or prompt handling changes for the Investment-Memo "Current News" use case, verify this call site still gets a complete JSON array back, since both features share the same two methods.**
